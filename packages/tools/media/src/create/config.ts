...
export type DefaultMediaOptions = {
	 ...
	 */
	fontFamily? : string
	core?       : Omit<CreateImageOptions, 'output'>
}

export const setDefaultMediaConfig =   ( opts: DefaultMediaOptions ): CreateImageValueFn => {
...
`
	...
	return async d => {
		...
		await d.createImage( d.setHTML( markup ), {
			output : d.outputFile,
			satori : {
				embedFont : true,
				width,
				height,
				...( satori || {} ),
				fonts     : await Promise.all( Object.values( fontWeight )
					.map( async w => ( {
						name   : fontFamily,
